...
Version: 4.0
Description: External JavaScript for form validation, Fetch API, cookies, local storage, and time event.
*/
...
function loadStates() {
    return fetch("states.html")
        ...
        .catch(function(error) {
            document.getElementById("state").innerHTML = '<option value="">State list failed to load</option>';
            console.log(error);
        });
}
...
function validateSsn() {
    ...
    if (!/^\d{9}$/.test(digitsOnly)) {
        showError("ssnError", "Social Security or ID must contain exactly 9 digits.");
        return false;
    }
...
function checkFirstNameCookie() {
    ...
    if (firstName !== "") {
        ...
        document.getElementById("newUserBox").innerHTML =
            '<input type="checkbox" id="notUser" onclick="startNewUser()"> ' +
            '<label for="notUser">Not ' + firstName + '? Click here to start as a new user.</label>';
    } else {
...
function saveFormData() {
    ...
    var fields = [
        "middleInitial", "lastName", "dob", "email", "phone",
        "addr1", "addr2", "city", "state", "zip", "userId",
        "healthRating", "symptoms"
    ];
    ...
    saveRadioValue("gender");
    saveRadioValue("vaccinated");
    saveRadioValue("insurance");
    saveCheckboxValues();
}
...
function loadFormData() {
    ...
    loadRadioValue("gender");
    loadRadioValue("vaccinated");
    loadRadioValue("insurance");
    loadCheckboxValues();
    updateHealthValue();
}
...
function loadRadioValue(name) {
    var saved = localStorage.getItem(name);

    if (saved !== null) {
        var radio = document.querySelector('input[name="' + name + '"][value="' + saved + '"]');
...
function saveCheckboxValues() {
    var checked = [];
    var boxes = document.querySelectorAll('input[name="history"]:checked');

    boxes.forEach(function(box) {
        checked.push(box.value);
    });

    localStorage.setItem("history", checked.join(","));
}
...
function loadCheckboxValues() {
    ...
    values.forEach(function(value) {
        var box = document.querySelector('input[name="history"][value="' + value + '"]');
